...
class CreateVoteProjectsTable extends Migration
{
	...
	public function up()
	{
		Schema::create('vote_projects', function(Blueprint $table) {
            ...
            $table->text('slide')->comment('投票主题幻灯片');
            $table->text('desc')->comment('投票说明');
            ...
            $table->integer('visitd')->comment('访问量');
            $table->integer('involved')->comment('参与者数量');
            $table->integer('voted')->comment('投票数量');
            $table->unsignedTinyInteger('vote_rule')->comment('投票规则编号');
            $table->timestamps();
		});
        ...
        Schema::create('vote_item', function(Blueprint $table) {
            ...
            $table->text('desc')->comment('图片描述');
            $table->integer('voted')->comment('投票数量');
            $table->unsignedTinyInteger('status')->comment('审核状态 0-未审核 1-审核通过 2 锁定');
            $table->timestamps();
        });
